...
NetTensor *Attention(NetTensor *x, int embedding_size, int hidden_size, int head_size, int cache_max, string name) {
    ...
    q = _RoPE({q}, PHI3ROPE, name + ".q_rope");
    k = _RoPE({k}, PHI3ROPE, name + ".k_rope");
...
void PHI3(Context *c, int vocab_size = 32000, int hidden_dim = 4096, int ffn_hidden_dim = 11008, int mutil_head_size = 32, int cache_max = 200) {
    auto *i = _Input(c);
...
int main(int argc, char **argv) {
    ...
    cmdParser.add<string>("model", 'm', "specify mllm model path", false, "../models/phi3_128.mllm");
    ...
    auto *c = c_ptr.get();
    PHI3(c, vocab_size, hidden_dim, ffn_hidden_dim, mutil_head_size, tokens_limit);
